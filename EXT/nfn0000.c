...
/* ------------------------------------------------------------------------------------------------------------ */
int nfn0000(commbuff_t   commbuff)
{

    int                 rc = ERR_NONE;
    nfn0000_ctx_t       _ctx;  
    nfn0000_ctx_t       *ctx = &_ctx;
...
static int a000_data_receive(nfn0000_ctx_t  *ctx, commbuff_t  commbuff)
{
    ...
    /* set commbuff  */
    memset((char *)ctx, 0x00, sizeof(nfn0000_ctx_t));
    ctx->cb = commbuff;
...
/* ------------------------------------------------------------------------------------------------------------ */
static int b000_init_proc(nfn0000_ctx_t *ctx)
{
    ...
    memcpy(SYSICOMM->call_svc_name, "NFN0000", 7);
...
static int c000_instsq_update(nfn0000_ctx_t     *ctx)
{
    ...
    if (SYS_DB_CHK_FAIL){
        ...
        ex_syslog(LOG_ERROR, "[APPL_DM] %s c000_instsq_update: UPDATE EXINSTSQ %d%d"
...
**/
    ...
    if (rc == ERR_ERR){
        ...
        ex_syslog(LOG_ERROR, "[APPL_DM]%s NFN0000: m000_ext_msg_send() ERROR %d [해결방안] 대외기관 G/W담당자 CALL", __FILE__, tperrno);
        return ERR_ERR;
    }
    ...
    if (rc == ERR_ERR){
        ex_syslog(LOG_ERROR, "[APPL_DM]%s NFN0000: NF_LOG_ERROR[UNIX->KFTC]", __FILE__);
    }
...
static int z100_log_insert(nfn0000_ctx_t    *ctx, char *log_data, int size, char io_flag,   char sr_flag)
{
    ...
    if (rc == ERR_ERR) {
        ex_syslog(LOG_ERROR, "[APPL_DM]%s NFN0000: z100_log_insert() NFLOG ERROR sysocbdb log_type :%d", __FILE__, sr_flag);
        sys_err_init();
        return ERR_NONE;
    }
    ...
    if (rc == ERR_ERR) {
        ex_syslog(LOG_ERROR, "[APPL_DM]%s NFN0000: z100_log_insert() NFLOG ERROR sysocbsi log_type :%d", __FILE__, sr_flag);
        sys_err_init();
        sysocbfb(&dcb);
        return ERR_NONE;
    }
