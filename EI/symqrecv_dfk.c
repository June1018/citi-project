...
/*  @file               symqrecv_dfk.pc
*   @file_type          pc source program
*   @brief              KTI ==> MQ ==> EI로 들어오는 PROCESS
*   @warn
*   @author
*   @version 
*   @dep_haeder
...
typedef struct symqrecv_dfk_ctx_s  symqrecv_dfk_ctx_t;
struct symqrecv_dfk_ctx_s {
    exmqparm_t      exmqparm;
...
char                g_pid[6 + 1];                       /* PROCESS  ID        */

/* ------------------------------------------ exported function  declarations --------------------------------- */
int                 symqrecv_dfk(commbuff_t    *commbuff);
...
static int          b100_mqparm_load(symqrecv_dfk_ctx_t    *ctx);
static int          c100_check_available_time(symqrecv_dfk_ctx_t    *ctx);
static int          d100_init_mqcon(symqrecv_dfk_ctx_t     *ctx);
static int          e100_get_mqmsg(symqrecv_dfk_ctx_t    *ctx);
static int          f100_mqmsg_proc(symqrecv_dfk_ctx_t      *ctx);
static int          f200_set_exparm(symqrecv_dfk_ctx_t      *ctx);
static int          g000_call_svc(symqrecv_dfk_ctx_t   *ctx);
static int          z200_mqmsg_update(symqrecv_dfk_ctx_t   *ctx);
int                 apsvrdone();
...
static int          a100_parse_custom_args((int argc,  char *argv[])
{
    ...
    /* ----------------------------------------------------------------------------------- */
        ...
        case 'g' :
            //strcpy(g_call_svc_name, optargs);
            break;
...
{
    int                 rc = ERR_NONE;
    symqrecv_dfk_ctx_t     _ctx = {0};111
    symqrecv_dfk_ctx_t     *ctx = &ctx;  
...
/* ------------------------------------------------------------------------------------------------------------ */
static int  f200_set_exparm(symqrecv_dfk_ctx_t  *ctx,   hcmihead_t         *hcmihead)
{
    int                 rc  = ERR_NONE;
    exi0331_t           exi0331 = {0};
...
/* ------------------------------------------------------------------------------------------------------------ */
static int   z900_error_rspn_proc(symqrecv_dfk_ctx_t    *ctx,   exmqmsg_001_t   *mqmsg_001)
{
