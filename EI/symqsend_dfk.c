/*  @file               symqsend_dfk.pc
*   @file_type          pc source program
*   @brief              KTI Channel ==> MQ ==> EI로 들어오는 PROCESS
*   @warn
*   @author
*   @version 
*   @dep_haeder
...
typedef struct symqsend_dfk_ctx_s  symqsend_dfk_ctx_t;
struct symqsend_dfk_ctx_s {
    char            tx_code[LEN_HCMIHEAD_TX_CODE + 1];
    exmqparm_t      exmqparm;
    mq_info_t       mqinfo;
...
symqsend_dfk_ctx_t  _ctx;
symqsend_dfk_ctx_t  *ctx = &_ctx;  
...
static int          b100_mqparm_load(symqsend_dfk_ctx_t    *ctx);
static int          d100_init_mqcon(symqsend_dfk_ctx_t     *ctx);
static int          e100_get_sendmsg(symqsend_dfk_ctx_t    *ctx);
static int          f100_put_mqmsg(symqsend_dfk_ctx_t      *ctx);
static int          g100_update_proc_code(symqsend_dfk_ctx_t   *ctx);
...
int apsvrinit(int argc, char argv[])
{
    ...
    SYS_DBG("SYMQSEND_DFK Initialize ");
...
/* ------------------------------------------------------------------------------------------------------------ */
int  symqsend_dfk(commbuff_t    *commbuff)
{
    int                 rc = ERR_NONE;
    int                 i;
    char                *cp;
    char                *hp;
    //symqsend_dfk_ctx_t     _ctx = {0};
    //symqsend_dfk_ctx_t     *ctx = &ctx;  

    SYS_TRSF;

    SYS_DBG(" ================SYMQSEND_DFK START =================== ");
